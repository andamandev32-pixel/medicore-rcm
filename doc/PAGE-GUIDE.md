...
| ส่วนเกริ่น "ที่มาของโครงการ" | ทั้ง 3 ชุดขึ้นต้นเหมือนกัน: ปก → `prfPainMap()` → `prfPainScope()` (`present-flows.js` §8) · เนื้อหา pain point 6 ข้อของลูกค้าอยู่ใน §8 **ที่เดียว** แก้ที่นั่นแล้วเปลี่ยนครบทุก deck |
