...
### 7B-1. ชุดสไลด์ (`present*.html`) — ทะเบียนอยู่ที่ `DS_DECKS`

| เรื่อง | กติกา |
|---|---|
| ทะเบียนชุดสไลด์ | `DS_DECKS` ใน `public/js/ds/ds-navbar.js` **ที่เดียว** — เมนู "นำเสนอ" และการ์ดบน `present-hub.html` อ่านจากที่นี่ทั้งคู่ |
| เพิ่มชุดใหม่ | คัดลอก `present-modules.html` เป็น shell → เขียนไฟล์สไลด์ → เพิ่ม 1 รายการใน `DS_DECKS` |
| ชื่อตัวแปร | ทุกไฟล์สไลด์ประกาศ `const PRESENT_SLIDES` ชื่อเดียวกัน — **ห้ามโหลดสองไฟล์ในหน้าเดียว** |
| ลำดับสคริปต์ | `present-flows.js` → `present-flows-modules.js` → ไฟล์สไลด์ → `present.js` · ไฟล์ผังต้องมาก่อนเสมอเพราะอาร์เรย์สไลด์เรียกฟังก์ชันวาดผังทันทีตอนสร้าง |
| `count` | ต้องตรงกับจำนวนสมาชิกจริง — **ห้ามเขียนจำนวนสไลด์ซ้ำที่อื่น** (`index.html` เคยเขียน 20 ทั้งที่มี 23 และลิงก์ `#page-N` ก็ชี้ผิดสไลด์หลังแทรกหน้า) |
| ลิงก์เข้าสไลด์ | ลิงก์ระดับ "ชุด" เท่านั้น **ห้ามลิงก์ `#page-N` จากหน้าอื่น** — เลขหน้าเลื่อนทุกครั้งที่แทรกสไลด์ |
| ส่วนเกริ่น "ที่มาของโครงการ" | ชุดที่เล่าข้อเสนอ (`present.html` · `present-exec.html` · `present-modules.html`) ขึ้นต้นเหมือนกัน: ปก → `prfPainMap()` → `prfPainScope()` (`present-flows.js` §8) · เนื้อหา pain point 6 ข้อของลูกค้าอยู่ใน §8 **ที่เดียว** แก้ที่นั่นแล้วเปลี่ยนครบทั้งสามชุด · **ข้อยกเว้น** ชุดรายงาน `present-report-1/2/3.html` ไม่มีส่วนเกริ่นและไม่โหลด `present-flows*.js` เพราะยกเนื้อหารายงานมาทั้งเล่ม ไม่ใช่การนำเสนอข้อเสนอ — ถ้าใส่เกริ่นเข้าไปจะกลายเป็น deck ที่เล่าเรื่องซ้ำ |
| ลิงก์ออกนอกระบบในเมนู | เอกสารฉบับเต็มที่ไม่ได้อยู่ใน `public/` (เช่นรายงานวิเคราะห์) ลงทะเบียนที่ `DS_REPORTS` ใน `ds-navbar.js` **ไม่ใช่** `DS_DECKS` — คนละความหมาย และ `DS_DECKS` ถูกนับเป็นจำนวนชุด/จำนวนหน้าบน `present-hub` · รายการที่ตั้ง `external: true` จะได้ `target="_blank"` + ป้าย ↗ อัตโนมัติ |
| ปัญหาซ้ำกันเอง | สไลด์ที่พูดเรื่องปัญหาอยู่เดิม (`present.html` "สาเหตุร่วม" · `present-exec.html` "ของเดิม") ถูกปรับให้เล่า **สาเหตุ/กระบวนการ** ไม่ใช่เล่าปัญหาซ้ำ — เพิ่มสไลด์ใหม่แล้วต้องรักษาแนวนี้ไว้ |

**ผัง SVG** (`present-flows.js` / `present-flows-modules.js`) — ไฟล์หลังใช้ `PRF` และ
`prfBox`/`prfArrow`/`prfMarkers`/`prfText`/`prfSvg` ของไฟล์แรกร่วมกัน **ห้ามประกาศซ้ำ**

| กติกา | เหตุผล |
|---|---|
| `id` ของ `<marker>` ต้องไม่ซ้ำทั้ง deck — prefix ทุกตัว | อยู่ใน document เดียวกัน ซ้ำเมื่อไรลูกศรใช้สีผิด |
| ผังเต็มสไลด์ **ไม่ส่ง** อาร์กิวเมนต์ `auto` ของ `prfSvg()` | จะได้ `.pr-svg` ที่ CSS วาง `position:absolute; inset:0` แล้วย่อพอดีกรอบ · ถ้าส่ง `true` จะได้ `height:auto` ซึ่งสูงเกิน `.pr-body` (`overflow:hidden`) แล้วโดนตัดท้ายผังทิ้ง |
| ไม่วาดหัวเรื่องซ้ำใน SVG | สไลด์มี `<h2>` ของตัวเองอยู่แล้ว · วาดซ้ำ = หัวเรื่องสองชั้นและกินพื้นที่ผัง |
| ไม่ลากเส้นย้อนกลับที่ระดับกึ่งกลางแถวกล่อง | เส้นจะพาดกลางกล่องแล้วบังข้อความ — ต้องอ้อมลงใต้ป้ายชื่อไฟล์ก่อน |
| ตัดบรรทัดภาษาไทยเอง | SVG ตัดบรรทัดให้ไม่ได้ · เกณฑ์: กว้างกล่อง ÷ (0.55 × font-size) |

### 7C. กติกาเฉพาะของหน้า `nhso-*`

หน้าชุดนี้ต้อง **ตรงกับเอกสาร สปสช.** เพราะผู้ฟังจำหน้าจอจริงได้ ถ้าตัวเลขเพี้ยนคือเสียความน่าเชื่อถือทั้งชุด

| กติกา | รายละเอียด |
|---|---|
| ลำดับโหลด mock | `mock-nhso.js` ต้องอยู่**ก่อน** `mock-claims.js` เสมอ — ตัวสร้างเคสเรียก `MockNhso.checkFiles()` ตอน normalize |
| ค่าคงที่มาจากไฟล์เดียว | ชื่อสถานะ · จำนวนฟิลด์ · เมทริกซ์กองทุน · ชื่อรายงาน อยู่ใน `mock-nhso.js` ที่เดียว ห้ามพิมพ์ซ้ำในหน้าจอหรือสไลด์ |
| อ้างที่มาทุกค่า | คอมเมนต์ต้องระบุ `[D2 น.xx]` (Overview 23 มิ.ย. 2569) หรือ `[D3 สไลด์ xx]` (Communication V4 3 ส.ค. 2569) |
| ค่าที่ยืนยันไม่ได้ต้องติดธง | ใส่ `verified: false` แล้วให้หน้าจอขึ้น `<sup>*</sup>` พร้อม `title` = `NHSO_UNVERIFIED_NOTE` — ห้ามแสดงรหัสที่ยืนยันไม่ได้แบบเงียบ ๆ |
| ผลรวมต้องกระทบยอด | ฟิลด์ 15 แฟ้มต้องรวมได้ **72 / 16 / 72 = 160** · 2 ถังรวมกันต้องเท่าจำนวนเคสทั้งหมด · เงินโอนเข้าบัญชี = จ่ายชดเชย + จ่ายเพิ่ม − เรียกคืน |
