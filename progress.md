# progress.md — บันทึกความคืบหน้า MediClear
...
- `c3df42d` แบรนด์ MediClear + โมดูลผู้ป่วยใน/ผู้บริหาร/ศูนย์รวมสไลด์
...
### 12 ส.ค. 2569 (รอบเจ็ด) — วิจัยตลาดโปรแกรมนอก HIS + วิเคราะห์คู่แข่ง บ.สาติ (Sati)

รอบนี้ไม่มีโค้ดฝั่งระบบ เป็นงานวิจัยตลาดล้วน + นำผลเข้าสไลด์ — ตั้งต้นจากคำถามว่า
"มีโปรแกรมที่ทำงานตรวจเวชระเบียนและส่งเบิกโดยแยกจาก HIS อยู่จริงไหม" คำตอบคือมี และมากกว่าที่เล่ม 1 ประเมินไว้

- **`doc/market-nonhis-claim-audit.html`** (ใหม่) — รายงานเล่ม 2 ใช้ระบบสีและโครง CSS ชุดเดียวกับเล่ม 1
  เพื่อให้เป็นชุดเดียวกัน เพิ่ม token `--rival` สีเดียวสำหรับ "หมึกของฝั่งเขา" —
  ทุกตัวเลขที่เป็นคำกล่าวอ้างของผู้ขายพิมพ์ด้วยสีนี้ ไม่ปนกับตัวเลขที่เราตรวจสอบเองได้
- **ข้อค้นพบที่แก้ข้อสรุปเดิม** — ตลาดมี 5 กลุ่มตามเจ้าของเครื่องมือ ไม่ใช่ "HIS vendor กับเรา":
  ① เครื่องมือของผู้จ่ายเอง (e-Claim · NHSO DP · Single Audit Platform · SSOP · FDH · App-C) ฟรีและบังคับใช้
  ② โปรแกรมชุมชน (RCM – Revenue Claim Manager, Data Audit, JSON Check Code) ฟรี ไม่มีผู้ดูแล
  ③ AI นอก HIS = **Sati** ④ โมดูลใน HIS ⑤ AI เอกสารฝั่งประกัน/TPA
  — **คู่แข่งที่ถูกที่สุดคือของฟรีจากผู้จ่าย ไม่ใช่บริษัทซอฟต์แวร์**
- **Sati** — สตาร์ทอัพเชียงใหม่ ก่อตั้งโดยแพทย์ · NLP+LLM+Agentic AI อ่านข้อความอิสระในชาร์ตแล้วเสนอรหัส
  ICD/DRG · ลูกค้า รพ.ตติยภูมิ 5 แห่ง · ต่อ HOSxP 4 สัปดาห์ · **ขายแบบแบ่งรายได้ ไม่ต้องลงทุนก่อน**
  · จับมือ MEDcury ตั้งแต่ ก.พ. 2568 · มีสินค้าฝั่งผู้จ่ายด้วย (fraud detection, policy check)
  ⚠️ ตัวเลขที่โฆษณา (62 ล้านบาท/รพ., 71%, 97%) เป็นคำกล่าวอ้างของผู้ขาย **ไม่มีการตรวจสอบอิสระ**
  และช่วงจริงที่ระบุในบทสัมภาษณ์คือ 5–62% ไม่ใช่ 62%
- **แกนกลยุทธ์ที่ได้** — Sati แก้ *Underclaim* (เก็บได้ไม่ครบ) เราแก้ *Denial + Clawback*
  (ส่งไม่ผ่าน และเก็บได้แล้วถูกเรียกคืน) · ข้อมูลราชการชี้ว่า **>80% ของ รพ. มี RW ตกหลัง audit**,
  สุ่มตรวจ 3% **ขยายผลได้ 33 เท่า/หักจ่าย 20%**, บอร์ด สปสช. อนุมัติ Audit 100% รพ.เอกชน
  และผู้จ่ายลงทุน AI ตรวจจับเอง 24 ล้านบาท — **จึงไม่ควรแข่งเป็น "AI ที่เก่งกว่า" แต่แข่งที่สายหลักฐาน**
- **สไลด์** — `present-exec-slides.js` +2 หน้า (8B แผนที่ตลาด 5 กลุ่ม · 8C Sati vs เรา) ·
  `present-slides.js` +1 หน้า (8B ความเสี่ยงถูกเรียกเงินคืน วางต่อจาก "สองวงจร") ·
  `DS_DECKS` count exec 16→18, ฉบับเต็ม 30→31 (modules เป็น 28 อยู่แล้วจากรอบหก)
- **ข้อที่ยังไม่มีคำตอบและต้องตอบก่อนเข้าตลาด** — โมเดลราคา ข้อเสนอในรายงานคือ outcome-based
  ที่ผูกกับตัวชี้วัดที่**ไม่ถูกเรียกคืนทีหลัง** (first-pass · เคสติด C ที่ลดลง · ยอดค้างที่ปิดได้ ·
  ผล audit ที่ไม่ถูกหัก) ต่างจากการแบ่งจาก "รายได้ที่เพิ่ม" ซึ่งวัดตอนส่ง แต่ความเสี่ยงมาถึงตอนถูกตรวจปีถัดไป

### 12 ส.ค. 2569 (รอบแปด) — ผัง data flow การตรวจสอบเวชระเบียน แยก IPD/OPD และรายกองทุน

รอบนี้ไม่มีโค้ดฝั่งระบบเช่นกัน เป็นการ **วาดสิ่งที่มีอยู่แล้วให้เห็นเป็นภาพ** ไม่ได้เพิ่มความสามารถ —
ตั้งต้นจากรายงานวิจัยกฎกรองเคลม ที่อธิบายการตรวจเป็น "6 ชั้น" แบบข้อความเรียงกัน
แต่ยังไม่เห็นว่าข้อมูลไหลจากไหนไปไหน และอะไรเป็นตัวกำหนดว่าเคสหนึ่งต้องตรวจอะไร

- **`public/js/present-flows-audit.js`** (ใหม่, prefix `pa*`) — ผัง SVG 3 ตัว
  `paAuditFlow()` ภาพรวม · `paAuditIpd()` · `paAuditOpd()` ·
  แยกไฟล์จาก `present-flows-modules.js` เพราะไฟล์นั้นประกาศสัญญาไว้ว่า "กล่องหนึ่ง = หน้าจอจริงหนึ่งหน้า"
  ส่วนชุดนี้กล่องคือ **ชั้นการตรวจและตารางข้อมูล** — ปนกันแล้วคนอ่านโค้ดรอบหน้าเข้าใจผิด
  ใช้ `PRF` / `prfBox` / `prfArrow` / `prfBadge` / `prfSvg` และ `pmBand` / `pmEdgeLabel` ซ้ำทั้งหมด
- **`present-modules-slides.js` +4 หน้า** (24 → 28) แทรกเป็นส่วน "เจาะลึก · การตรวจสอบเวชระเบียน"
  คั่นระหว่างส่วนงานที่ 5 กับสรุป pain point — เพราะเนื้อหาคร่อม 3 ส่วนงาน
  ถ้ายัดใต้ส่วนใดส่วนหนึ่งโครง "5 ส่วนงาน" จะเสียรูป
- ⭐ **สาระหลักคือ "สองคีย์คนละแกน"** ซึ่ง `reference.sql:336-341` เตือนไว้แล้วแต่ยังไม่เคยมีภาพ:
  `fund_key` (12 กองทุน) กำหนดว่า**ต้องส่งแฟ้มไหน** → ชั้น FILES ·
  `payer_key` (6 สิทธิ) กำหนดว่า**ใช้กฎข้อไหน เอกสารอะไร เรตเท่าไร** → คลังกฎ
  ในผังภาพรวมจึงวาดเส้นของสิทธิให้ลอดช่องว่างระหว่างคอลัมน์ลงไปหาคลังกฎ **โดยไม่แตะ 8 ชั้นเลย**
- **แก้คำผิดที่ค้างอยู่ในสไลด์เดิม 3 จุด** — ข้อความ "6 กองทุน — บัตรทอง · ข้าราชการ · ..."
  ทั้งหกคือ **สิทธิ** ไม่ใช่กองทุน (กองทุนมี 12 และไม่มี "ประกันเอกชน" อยู่ในนั้น)
  แก้เป็น "6 สิทธิ" ที่ `foot` สไลด์ IPD · ตาราง "สิ่งที่จะได้" · ตารางสรุป pain point
  พร้อมเพิ่มกติกาห้ามใช้สลับกันไว้ในหัวไฟล์
- **แก้ 8 ชั้น ไม่ใช่ 6** — รายงานวิจัยอ้างอิงสถานะ ณ commit `de8df80` ซึ่งยังไม่มี PROC/DRG
  สไลด์ใหม่ยึดของจริงตาม §1 ของเอกสารนี้
- **`DS_DECKS` count modules 24 → 28** — บันทึกรอบเจ็ดเขียนว่า "modules เป็น 28 อยู่แล้วจากรอบหก"
  แต่ของจริงที่ HEAD คือ 24 ทั้งในอาร์เรย์และใน `DS_DECKS` · ตอนนี้ทั้งสามชุดตรงกับจำนวนสไลด์จริงแล้ว
  (exec 18 · modules 28 · ฉบับเต็ม 31)
- **ยังไม่ได้แก้ และตั้งใจไม่แก้** — ป้ายแท็บในหน้า `ipd-audit` ยังชื่อ "เงื่อนไขกองทุน"
  ทั้งที่อ่านจาก `IPD_FUND_RULES[payer]` คือเงื่อนไขราย**สิทธิ** · จะเปลี่ยนต้องแก้ทั้งหน้าจอ
  ไม่ใช่แก้เฉพาะสไลด์ ไม่งั้น deck กับหน้าจอจะเรียกคนละชื่อ — ดู §4 ข้อ 12

### ผลการทดสอบที่ผ่านแล้ว
- **ผัง data flow ทั้ง 3 ตัวตรวจเรขาคณิตแบบ static ผ่าน** — ไม่มีตัวอักษรล้นกล่อง ไม่มีกล่องทับกัน
  ไม่มีข้อความล้น viewBox · `<marker>` ทั้ง deck 36 ตัว **ไม่มี id ซ้ำ** (id ซ้ำ = ลูกศรได้สีผิดแบบเงียบ ๆ)
- **ตัวเลขบนสไลด์ตรงกับ API จริงทุกช่อง** — `/api/reference/fund-files` เมทริกซ์ 12 กองทุนตรงทั้งตาราง ·
  `/api/rules/coverage` = ACTIVE 21 / executable 10 / 48% · `/api/reference/mra` = MRA-2563,
  12 องค์ประกอบ, บังคับเสมอ 7, `criteria_pending` 11 (ถอดเกณฑ์ย่อยแล้วเฉพาะองค์ประกอบที่ 1)
- `npm run check:rules` ผ่าน — คลังกฎกับตัวตรวจในโค้ดตรงกัน
...
### เอกสาร
- **รายงานวิเคราะห์ เล่ม 1** (มาตรฐานกฎ 6 ชั้น · แหล่งฐานข้อมูล · คู่แข่ง BMS i-Claim / HA.OS / MEDcury + SWOT): https://claude.ai/code/artifact/cb207927-6e5b-4e95-98fc-d216263f4bd7
- **รายงานวิเคราะห์ เล่ม 2** (ตลาดโปรแกรมนอก HIS 5 กลุ่ม · โปรไฟล์ บ.สาติ (Sati) · ความเสี่ยงถูกเรียกเงินคืน · battle card): https://claude.ai/code/artifact/f1d336be-2194-479f-b702-389441447f63
  · ต้นฉบับอยู่ในเรพที่ `doc/market-nonhis-claim-audit.html` — แก้ไฟล์นั้นแล้ว publish ซ้ำที่ URL เดิม
- **รายงานวิเคราะห์ เล่ม 3** (เดินเคส AN 691209 ทีละชั้น + ผลรัน validator จริง FAIL→PASS): https://claude.ai/code/artifact/5b9a12c4-894f-4b57-9b25-6697efc7ab87
  · ต้นฉบับ `doc/case-walkthrough-vs-sati.html` · เคสที่รันได้จริงอยู่ที่ `doc/examples/` พร้อม README บอกจุดผิด 6 จุดและรหัสที่ควรได้
  ⚠️ ถ้าแก้ validator/suggester ให้รัน 2 ไฟล์ใน `doc/examples/` ซ้ำ — รายงานเล่ม 3 อ้างผลลัพธ์ไว้ตรงตัว (FAIL 4/3/1/2 และ PASS 0/0/1/1) ถ้าเลขเปลี่ยนต้องแก้รายงานด้วย
  ⚠️ เล่ม 2 **แก้ข้อสรุปของเล่ม 1**: ข้อความ "ยังไม่มีใครยืนตำแหน่งเดียวกับเรา" ใช้ไม่ได้แล้ว —
  Sati ยืนตำแหน่ง HIS-agnostic + ตรวจก่อนส่งเบิกอยู่ก่อน มีลูกค้า รพ.ตติยภูมิ 5 แห่ง และขายแบบแบ่งรายได้
...
| 12 | **เปลี่ยนคำว่า "เงื่อนไขกองทุน" ในหน้า `ipd-audit` เป็น "เงื่อนไขรายสิทธิ"** — แท็บนั้นอ่านจาก `IPD_FUND_RULES[payer]` คือเงื่อนไขราย**สิทธิ** ไม่ใช่ราย**กองทุน** (กองทุนมี 12 · สิทธิมี 6 — `reference.sql:336-341`) | ต้องแก้พร้อมกันทั้ง `ipd-audit.html` (ป้ายแท็บ) · `ipd-audit.js` · `mock-ipd.js` (ชื่อค่าคงที่) · สไลด์ที่อ้างชื่อแท็บ · สูตรคะแนน 60/20/20 — แก้เฉพาะสไลด์ไม่ได้ ไม่งั้น deck กับหน้าจอเรียกคนละชื่อ ผิดกติกาของ deck ชุดนี้ · รอบแปดแก้ไปแล้วเฉพาะข้อความที่นับ "6 กองทุน" ซึ่งผิดข้อเท็จจริงชัด ๆ |
