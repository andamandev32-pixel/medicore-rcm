...
/* ══════════════════════════════════════════════════════════
   ...
   ⚠️ สัดส่วน viewBox 1300×522 (≈2.49) จงใจให้เท่ากับพื้นที่ .pr-body ของหน้าปก
      ...
      เดิม 1300×430 (≈3.02) จึงเสียพื้นที่แนวตั้งไป ~13%
...
function prfCoverage() {
    const W = 1300, H = 522, PAD = 14, GAP = 30, N = 4;
    const BW = (W - PAD * 2 - GAP * (N - 1)) / N;          /* ≈ 295.5 */
    const xs = i => PAD + i * (BW + GAP);
    const cx = i => xs(i) + BW / 2;

    /* ระยะแนวตั้ง — ช่องว่างรอบรางจงใจกว้าง 34–40
       เพื่อให้เห็นว่าเส้นออกจากกล่องไหน ไปรวมที่ราง แล้วไปต่อที่แถบไหน */
    const TOP_Y = 2, TOP_H = 56;
    const RAIL_UP = 98;                                    /* ห่างแถบบน 40 · ห่างหัวกล่อง 40 */
    const CY = 138, CH = 254, CBOT = CY + CH;              /* กล่องจบที่ 392 */
    const RAIL_DOWN = 426;                                 /* ห่างท้ายกล่อง 34 */
    const BOT_Y = 460, BOT_H = 60;                         /* จบที่ 520 เหลือขอบล่าง 2 */

    /* เนื้อในกล่อง: หัวเรื่อง + 3 ชั้น ชั้นละหัวข้อ 1 บรรทัด เนื้อ 2 บรรทัด
       เกณฑ์ความยาว: (BW − ขอบ 2×15) ÷ (0.55 × 14) ≈ 34 อักษรไทยต่อบรรทัด */
    const HEAD_H = 58, PADX = 15, LBL_H = 17, LINE_H = 18, BLK_GAP = 11;
    ...
    const cols = [
        ...
        { t: 'ส่งเบิก NHSO',
          ...
          b: [['ทุกชุดที่ส่ง 15 แฟ้ม 160 ช่องข้อมูล', 'Statement เทียบเงินที่โอนเข้าจริง'],
              ['รู้ยอดพึงรับ–พึงจ่ายทุกสิ้นเดือน', 'ตามเก็บเป็นรายเคสได้ ไม่ตกหล่น'],
              ['เดิมกระทบยอดด้วยมือ ไม่รู้ว่า', 'เคสไหนยังไม่ได้เงิน']] },
    ];
    ...
    g += prfText(W / 2, TOP_Y + TOP_H / 2,
        ['ภาพรวมผู้บริหาร — อ่านทั้ง 4 เส้นงานในหน้าเดียว · KPI ทุกช่องกดดูสูตรและเจาะถึงรายเคสได้'],
        { fs: 21, fill: PRF.onDark, w: 800 });
    ...
    cols.forEach((c, i) => {
        ...
        g += prfText(x + BW / 2, CY + HEAD_H / 2 - 2, [c.t], { fs: 21, fill: c.ic, w: 800 });
        ...
        c.b.forEach((lines, k) => {
            ...
            g += prfText(x + PADX + 9, y + LBL_H / 2, [LBLS[k]],
                { fs: 12.5, fill: c.ic, w: 800, anchor: 'start', op: .95 });
            g += prfText(x + PADX, y + LBL_H + lines.length * LINE_H / 2, lines,
                { fs: 14, fill: PRF.ink, w: 500, lh: 1.28, anchor: 'start', op: .86 });
    ...
    return prfSvg(W, H, g, 'ขอบเขตของระบบ สี่เส้นงาน แต่ละเส้นบอกว่าดูอะไรได้ ได้ผลอะไร แก้ปัญหาเดิมข้อไหน');
}
